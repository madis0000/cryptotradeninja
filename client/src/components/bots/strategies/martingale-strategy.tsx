...
export function MartingaleStrategy({ 
  ...
  const { user } = useAuth();
  const { subscribeToBalance, unsubscribeFromBalance, balances } = useMarketWebSocket();
  ...
  // Get real-time balance for selected symbol
  const realtimeBalance = balances[`${selectedExchangeId}_USDT`];
