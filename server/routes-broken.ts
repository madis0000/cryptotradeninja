    ...
    try {
      ...
      if (bot.strategy === 'martingale' && bot.status === 'active') {
        ...
        try {
          ...
          // Trading logic will be handled by dedicated service
    ...
    try {
        ...
        for (const order of pendingOrders) {
          if (order.exchangeOrderId && order.status === 'placed') {
            try {
              ...
              console.log(`[ORDER] Cancelling order ${order.exchangeOrderId} for bot ${bot.id}`);
      ...
      if (activeCycle && activeCycle.totalQuantity && parseFloat(activeCycle.totalQuantity) > 0) {
        ...
        try {
          ...
          console.log(`[LIQUIDATION] Processing liquidation for bot ${botId}`);
